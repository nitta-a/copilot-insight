...
├── native-parser/                # NAPI-RS Rust addon (optional native log parser)
│   ├── Cargo.toml                # Rust package manifest (napi + napi-derive + serde_json)
│   ├── build.rs                  # napi-build script required by NAPI-RS
│   └── src/
│       └── lib.rs                # NativeStats (#[napi(object)]) + parse_log_chunk / parse_log_file_native
├── src/
...
│   │   ├── logContentParser.ts       # Top-level parser coordinator (delegates to parsers/ sub-modules; uses native addon when available)
│   │   ├── nativeBridge.ts           # Thin NAPI-RS bridge: loadNativeModule / parseLogChunkNative / parseLogFileNative
...
1. **`src/log/copilotLogParser.ts`** — reads `.log` files from VS Code's extension host log directory, parses both JSON-embedded lines (via `parsers/jsonLogParser.ts`) and plain-text lines (via `parsers/textLogParser.ts`), and accumulates `CopilotUsageStats`. When the native addon is available `logContentParser.ts` delegates file parsing to `nativeBridge.ts` for improved performance. Also coordinates chat session title resolution and CLI log ingestion.
...
- **`src/log/nativeBridge.ts`** — thin NAPI-RS bridge: `loadNativeModule()` lazily loads the compiled `.node` addon (built by `npm run build:native`), `parseLogChunkNative()` parses a raw string, and `parseLogFileNative()` passes a file path directly to Rust (no Node.js file I/O). Returns `null` gracefully when the addon has not been built.
